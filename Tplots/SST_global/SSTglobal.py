...
#    bolat = np.logical_and(LATS[:,0]>=minlat-2,LATS[:,0]<=maxlat+1)
#    bolon = np.logical_and(LONS[50,:]>=minlon,LONS[50,:]<=maxlon+2)    
...
lonticks = [-90,0,90,180,270, 360]

#variables
var = 'SST' #'vel'#'VNT'#'vel'# 'SSH' 'SALT' 'MLD'
gw = True
...
#    cmap='nipy_spectral'
vsbath = [5,35]
contours = [7.5,12.5,17.5,22.5,27.5,32.5]
...
plt.title('0.1$^{\circ}$ resolution', fontsize=fs)
...
# if labels must be added to contours:
#CS.levels = [nf(val) for val in CS.levels]
#fmt = '%r '
#ax2.clabel(CS, CS.levels, inline=True, fmt=fmt, fontsize=10)

land = np.full(bath38.shape, np.nan); land[bath38==0] = 1;#land[bath38<=0] = 1;
plt.pcolormesh(lons38, lats38, land,  
               vmin=0, vmax=1.6,cmap='binary',
                     transform=ccrs.PlateCarree(),zorder=2
                     )

plt.scatter(paleolons,paleolats, marker='+', s=1000, color='k', 
            zorder=3000,transform=ccrs.PlateCarree())

for tx in range(len(paleolons)):
    plt.text(paleolons[tx]+1,paleolats[tx]+1, sitenames[tx], fontsize=15,
...
plt.savefig('meanSST_global.png', dpi=300,bbox_inches='tight',pad_inches=0)
plt.show()